...
def PRegAction(request):
	...
	con=pymysql.connect(host="localhost",user="root",password="root",database="doctor-patient")
	cur=con.cursor()
...
def bookslot(request):
	...
	data=cur.fetchall()
	strdata="<form action='/BookAction'><table></tr><th>Select Doctor:</th><td><select name='doctor'>"
	for i in data:
          strdata+="<option value='"+str(i[7])+"'>"+str(i[1])+","+str(i[7])+"</option>"
	strdata+="</select></td></tr><tr><th>Description</th><td><input type='text' name='content' required></td></tr><tr><th>Date</th><td><input type='date' name='date' required></td></tr><tr><th>time</th><td><input type='text' name='time' required></td></tr><tr><th></th><td><input type='submit' value='Book Now'></td></tr></table></form>"
...
def BookAction(request):
	...
	conn=pymysql.connect(host="localhost",user="root",password="root",database="doctor-patient")
	curr=conn.cursor()
