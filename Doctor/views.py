...
def viewrequest(request):
	...
	for i in data:
		strdata+="<tr><td>"+str(i[2])+"</td><td>"+str(i[3])+"</td><td>"+str(i[4])+"</td><td>"+str(i[5])+"</td><td>"+str(i[6])+"</td><td><a href='/doctor/confirm?did="+str(i[1])+"&date="+str(i[4])+"&time="+str(i[5])+"'>Confirm</a></td></tr>"
...
def confirm(request):
	...
	con=pymysql.connect(host="localhost",user="root",password="root",database="doctor-patient")
	cur=con.cursor()
	i=cur.execute("update bookslot set status='Confirmed' where specialization='"+doctorid+"'")
	con.commit()
	if i>0:
